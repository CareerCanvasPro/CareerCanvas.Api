...
import ServiceCaller from "./axios";
...
class AuthController {
    ...
    try {
      try {
        ...
        if (statusCode === 200) {
          ...
          return res
            .status(statusCode)
            .json({ message: "User is not confirmed. OTP resend initiated." });
        } else {
          return res
            .status(statusCode)
            .json({ message: response?.__type || "Sign-in failed" });
        }
      } catch (err) {
  ...
  resendEmailVerification = async (req: Request, res: Response) => {
    ...
    try {
      const response = await cognitoService.ResendOtpForEmailConfirmation(
        req.body.username
      );
  ...
  changeEmailRequest = (req: Request, res: Response) => {
    ...
    cognitoService
      .changeEmailRequest(accessTocken, email)
      .then((response) => {
        const responseMeta = response.$metadata;
        const statusCode = response.$metadata.httpStatusCode;
  ...
  changeEmailVerify = async (req: Request, res: Response) => {
    ...
    try {
      const response = await cognitoService.changeEmailVerify(
        accessToken,
        code
      );
      const responseMeta = response.$metadata;
  ...
  getIpFromRequest = async (req) => {
    try {
      ...
      const forwardedForHeader: string | undefined =
